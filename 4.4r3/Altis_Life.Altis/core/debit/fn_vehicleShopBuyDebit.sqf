...
try {
	if (CASH < _purchasePrice) then {
		if (BANK < _purchasePrice) then {
			hint parseText format [localize "STR_Shop_Veh_NotEnough",[_purchasePrice - BANK] call life_fnc_numberText];
			throw "No money";
		};
	};
			
	if ((BANK > _purchasePrice) && (CASH < _purchasePrice)) then {
